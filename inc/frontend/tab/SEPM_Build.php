<?php
		global $wpdb;
		$id = $_REQUEST['id'];
		$db_config = "SELECT * FROM sepm_build_file WHERE sepm_id=$id";
...
<div class="sepm_plannig_content edit_project_sub_content">
	<div class="container">
		<div class="row">
			...
			<div class="col-sm-8">
				<form id="build_form" action="" method="post" enctype="multipart/form-data">
				...
				<?php foreach( $results as $result ) {
									...
									<tr class="build_data">
										...
										<td>
											<span class='build_delete' data-id='<?php echo $id;?>' data-name='<?php echo $file_name;?>'>Delete</span>
										</td>
									</tr>
				<?php 		} ?>
			...
			<div class="col-sm-2">
				...
				<?php 
					if (isset($_POST['complete_build_step']) && isset($_POST['action']) && $_POST['action'] == "update_theme"){
						if (wp_verify_nonce($_POST['sepm_plugin_frontend'],'update-options')){ 
							update_option('complete_build_step',$_POST['complete_build_step']);
						}
					}
				?>
				...
				<form id="save-theme" name="save-theme" action="" method="post">
					...
					<input type="hidden" name="complete_build_step" value="sepmcomp" />
...
<script type="text/javascript">
	...
	//   var imgElement_src = $( '.sepm_file_name' ).attr("href");
	   ...
	   if (confirmalert == true) {
		  // AJAX Request
		  $.ajax({
			url: '<?php echo plugins_url('ajax/build_delete.php', __FILE__);?>',
			type: 'POST',
			data: { id:deleteid, name: imgElement_src },
			success: function(response){
			  if(response == 1){
		// Remove row from HTML Table
		$(el).closest('tr.build_data').css('background','tomato');
		$(el).closest('tr.build_data').fadeOut(600,function(){
		   $(this).remove();
		});
			  }else{
	...
	$("#build_form").on('submit',(function(e) {
	  ...
	  $.ajax({
	  // url: "ajaxupload.php",
	   url: "<?php echo plugins_url('ajax/build_submit.php', __FILE__);?>",
	   type: "POST",
	   ...
	   processData:false,
	   beforeSend : function(){
		$("#sepm_build_hide").fadeOut();
	   // $("#err").fadeOut();
	   },
	   success: function(data){
		  $("#update_build_div").html(data);
		if(data=='invalid'){
		 // invalid file format.
