...
//$insert = $db->query("INSERT sepm_plan_file (name,email,file_name) VALUES ('".$name."','".$email."','".$path."')");
//echo $insert?'ok':'err';
...
global $wpdb;
$wpdb->insert('sepm_plan_file',array( 'sepm_id'=>$sepm_id, 'file_name'=>$file_loc,) );

// Loop

$db_config = "SELECT * FROM sepm_plan_file WHERE sepm_id=$sepm_id";
		//$db_config = "SELECT * FROM sepm_plan_file";
		...
		foreach( $results as $result ) {
				...
				?>
					<tr class="plan_data">
						...
						<td><span class='p_delete' data-id='<?php echo $id;?>' data-name='<?php echo $file_name;?>'>Delete</span></td>
					</tr>
<?php 		} 
...
<script type="text/javascript">
	...
	//   var imgElement_src = $( '.sepm_file_name' ).attr("href");
	   ...
	   if (confirmalert == true) {
		  // AJAX Request
		  $.ajax({
			url: '<?php echo plugins_url('plan_delete.php', __FILE__);?>',
			type: 'POST',
