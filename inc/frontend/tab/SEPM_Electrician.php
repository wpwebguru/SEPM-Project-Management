<?php
		global $wpdb;
		$id = $_REQUEST['id'];
		$db_config = "SELECT * FROM sepm_electrician_file WHERE sepm_id=$id";
...
<div class="sepm_plannig_content edit_project_sub_content">
	<div class="container">
		<div class="row">
			...
			<div class="col-sm-8">
				<form id="electrician_form" action="" method="post" enctype="multipart/form-data">
                ...
                <tbody id="sepm_electrician_hide">
				...
				<?php foreach( $results as $result ) {
									...
									<tr class="elect_data">
										...
										<td>
											<span class='electrician_delete' data-id='<?php echo $id;?>' data-name='<?php echo $file_name;?>'>Delete</span>
										</td>
									</tr>
				<?php 		} ?>
				...
				<tbody id="update_electrician_div">
				</tbody>
				</table>
			...
			<div class="col-sm-2">
				...
				<?php 
					if (isset($_POST['complete_elect_step']) && isset($_POST['action']) && $_POST['action'] == "update_theme"){
						if (wp_verify_nonce($_POST['sepm_plugin_frontend'],'update-options')){ 
							update_option('complete_elect_step',$_POST['complete_elect_step']);
						}
					}
				?>
				...
				<form id="save-theme" name="save-theme" action="" method="post">
					...
					<input type="hidden" name="complete_elect_step" value="sepmcomp" />
...
<script type="text/javascript">
	$(document).ready(function(){
	 ...
	 // Delete 
	 $('.electrician_delete').click(function(){
	   var el = this;
	...
	//   var imgElement_src = $( '.sepm_file_name' ).attr("href");
	   ...
	   if (confirmalert == true) {
		  // AJAX Request
		  $.ajax({
			url: '<?php echo plugins_url('ajax/electrician_delete.php', __FILE__);?>',
			type: 'POST',
			data: { id:deleteid, name: imgElement_src },
			success: function(response){
			  if(response == 1){
		// Remove row from HTML Table
		$(el).closest('tr.elect_data').css('background','tomato');
		$(el).closest('tr.elect_data').fadeOut(600,function(){
		   $(this).remove();
		});
			  }else{
	...
	$("#electrician_form").on('submit',(function(e) {
	  e.preventDefault();
	  $.ajax({
	  // url: "ajaxupload.php",
	   url: "<?php echo plugins_url('ajax/electrician_submit.php', __FILE__);?>",
	   type: "POST",
	   ...
	   processData:false,
	   beforeSend : function(){
		$("#sepm_electrician_hide").fadeOut();
	   // $("#err").fadeOut();
	   },
	   success: function(data){
		  $("#update_electrician_div").html(data);
		if(data=='invalid'){
		 // invalid file format.
		 $("#err").html("Invalid File !").fadeIn();
		}else{
		 $("#electrician_form")[0].reset(); 
		}
		  },
		 error: function(e) {
		$("#err").html(e).fadeIn();
